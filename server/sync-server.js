...
const write=db=>fs.writeFileSync(DATA_FILE,JSON.stringify(db,null,2),'utf8');
...
const server=http.createServer(async(req,res)=>{
 ...
 try{
  ...
  if(req.method==='POST'&&req.url==='/api/sync/events'){
   ...
   if(!envelope.id||!envelope.deviceId||!event?.id||!event?.type)return json(res,400,{error:'بيانات المزامنة غير مكتملة'});
   const db=read(),existing=db.events[event.id];
