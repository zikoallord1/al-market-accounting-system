...
import { getCurrentStock } from '../services/eventEngine';
...
export default function Inventory({ state }) {
  ...
  const rows=useMemo(()=> (state.items||[]).map(item=>{
    ...
    return {...item,balance,inbound,outbound,value:balance*Number(item.cost||0),movementCount:movements.length};
  ...
  return <section className="page-card">
    ...
    <div className="table-wrap"><table><thead><tr><th>الصنف</th><th>الوحدة</th><th>الرصيد الحالي</th><th>الوارد</th><th>الصادر</th><th>التكلفة</th><th>قيمة المخزون</th><th>الحركات</th></tr></thead>
    <tbody>{rows.map(x=><tr key={x.id}><td>{x.name}</td><td>{x.baseUnit||x.unit}</td><td>{x.balance.toLocaleString('ar-YE')}</td><td>{x.inbound.toLocaleString('ar-YE')}</td><td>{x.outbound.toLocaleString('ar-YE')}</td><td>{Number(x.cost||0).toLocaleString('ar-YE')}</td><td>{x.value.toLocaleString('ar-YE')} ر.ي</td><td><History size={16}/> {x.movementCount}</td></tr>)}{!rows.length&&<tr><td colSpan="8" className="empty">لا توجد بيانات مخزون</td></tr>}</tbody></table></div>
  </section>;
}
