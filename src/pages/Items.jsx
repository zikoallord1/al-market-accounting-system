...
export default function Items({ state, setState }) {
  ...
  const reset = () => { setEditing(null); setForm({ name: '', barcode: '', unit: 'حبة', cost: '', price: '', stock: '' }); };
  const save = () => {
    if (!form.name.trim()) return;
    if (editing) {
      setState({ ...state, items: state.items.map(x => x.id === editing ? { ...x, ...form, cost: Number(form.cost||0), price: Number(form.price||0), stock: Number(form.stock||0), updatedAt: new Date().toISOString() } : x) });
    } else {
  ...
  const edit = (item) => setForm({name:item.name,barcode:item.barcode||'',unit:item.unit||'حبة',cost:item.cost||'',price:item.price||'',stock:item.stock||''}) || setEditing(item.id);

  return <section className="page-card">
    <div className="page-head"><div><h2>الأصناف</h2><p>إدارة الأصناف والوحدات والأسعار والأرصدة</p></div><Package size={28}/></div>
    ...
    <div className="form-grid compact">
      ...
      <input placeholder="الوحدة الأساسية" value={form.unit} onChange={e=>setForm({...form,unit:e.target.value})}/>
    ...
    <div className="table-wrap"><table><thead><tr><th>الصنف</th><th>الباركود</th><th>الوحدة</th><th>التكلفة</th><th>البيع</th><th>الرصيد</th><th>إجراءات</th></tr></thead><tbody>
      {filtered.map(x=><tr key={x.id}><td>{x.name}</td><td>{x.barcode||'—'}</td><td>{x.unit}</td><td>{Number(x.cost).toLocaleString('ar-YE')}</td><td>{Number(x.price).toLocaleString('ar-YE')}</td><td>{Number(x.stock).toLocaleString('ar-YE')}</td><td><button className="icon-btn" title="تعديل" onClick={()=>edit(x)}><Pencil size={16}/></button></td></tr>)}
